app.registerCtrl('GAInvoiceConfirmController', function ($scope, $rootScope, $location, $timeout, $sessionStorage, $http, $route, AuthData, $log, $ngConfirm, $filter, $window, SvTransportService) {
    AuthData.authenticationData.IsAuthenticated = true;
    $rootScope.ScreenName = "GA Invoice";
    $scope.GAInvoiceID = localStorage.getItem("GAInvoiceID");

    $scope.getInVoiceDetails = function () {
        $scope.InvoiceDetails = [];
        $scope.TotalQuantity = 0;
        $scope.TotalIncoiceAmount = 0;
        var invoicedata = SvTransportService.getInvoiceDataByIDService(parseInt($scope.GAInvoiceID), "GA");
        invoicedata.then(function (result) {
            if (result.data != null) {
                $scope.SvtDetails = result.data[0];
                for (var i = 0; i < result.data.length; i++) {
                    $scope.InvoiceDetails.push({
                        "DcDate": result.data[i].DCDate,
